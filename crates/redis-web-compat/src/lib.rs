...
pub const CANONICAL_MINIMAL_CONFIG_NAME: &str = "redis-web.min.json";
pub const LEGACY_CONFIG_NAME: &str = "webdis.json";
pub const LEGACY_MINIMAL_CONFIG_NAME: &str = "webdis.min.json";
...
impl InvocationKind {
    ...
    pub fn default_minimal_config_name(self) -> &'static str {
        match self {
            Self::Canonical => CANONICAL_MINIMAL_CONFIG_NAME,
            Self::LegacyAlias => LEGACY_MINIMAL_CONFIG_NAME,
        }
    }
...
pub fn resolve_default_config(kind: InvocationKind) -> String {
    match kind {
        InvocationKind::Canonical => {
            ...
            } else if Path::new(CANONICAL_MINIMAL_CONFIG_NAME).exists() {
                CANONICAL_MINIMAL_CONFIG_NAME.to_string()
