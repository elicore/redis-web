...
if [[ -f "$DIST_HIREDIS_DIR/lib/libhiredis.dylib" ]]; then
  ...
  [[ -f "$DIST_HIREDIS_DIR/lib/libhiredis_ssl.dylib" ]] && LIBHIREDIS_SSL_PATH="$DIST_HIREDIS_DIR/lib/libhiredis_ssl.dylib"
elif [[ -f "$DIST_HIREDIS_DIR/lib/libhiredis.so" ]]; then
  LIBHIREDIS_PATH="$DIST_HIREDIS_DIR/lib/libhiredis.so"
  [[ -f "$DIST_HIREDIS_DIR/lib/libhiredis_ssl.so" ]] && LIBHIREDIS_SSL_PATH="$DIST_HIREDIS_DIR/lib/libhiredis_ssl.so"
else
...
PROVIDED_SSL_SYMS_FILE="$ARTIFACTS_DIR/hiredis-ssl-provided-symbols.txt"
PROVIDED_ALL_SYMS_FILE="$ARTIFACTS_DIR/hiredis-provided-all-symbols.txt"
...
UPSTREAM_PROVIDED_SSL_SYMS_FILE="$ARTIFACTS_DIR/hiredis-upstream-ssl-provided-symbols.txt"
UPSTREAM_MISSING_SSL_SYMS_FILE="$ARTIFACTS_DIR/hiredis-ssl-missing-vs-upstream-symbols.txt"
...
extract_defined_symbols() {
  ...
  if [[ "$(uname -s)" == "Darwin" ]]; then
    nm -gU "$lib_path" | awk '{print $3}' | sed 's/^_//' | sed '/^$/d' | sort -u > "$out_file"
  else
    nm -D --defined-only "$lib_path" | awk '{print $3}' | sed 's/^_//' | sed '/^$/d' | sort -u > "$out_file"
  fi
}
...
extract_defined_symbols "$LIBHIREDIS_PATH" "$PROVIDED_SYMS_FILE"
if [[ -n "$LIBHIREDIS_SSL_PATH" ]]; then
  extract_defined_symbols "$LIBHIREDIS_SSL_PATH" "$PROVIDED_SSL_SYMS_FILE"
else
  : > "$PROVIDED_SSL_SYMS_FILE"
fi

grep -E '^(redis|sds|hi_|hiredis)' "$PROVIDED_SYMS_FILE" > "$PROVIDED_SYMS_FILE.filtered" || true
mv "$PROVIDED_SYMS_FILE.filtered" "$PROVIDED_SYMS_FILE"
grep -E '^(redis|sds|hi_|hiredis)' "$PROVIDED_SSL_SYMS_FILE" > "$PROVIDED_SSL_SYMS_FILE.filtered" || true
mv "$PROVIDED_SSL_SYMS_FILE.filtered" "$PROVIDED_SSL_SYMS_FILE"
cat "$PROVIDED_SYMS_FILE" "$PROVIDED_SSL_SYMS_FILE" | sort -u > "$PROVIDED_ALL_SYMS_FILE"

comm -23 "$REQUIRED_SYMS_FILE" "$PROVIDED_ALL_SYMS_FILE" > "$MISSING_SYMS_FILE"
...
(
  ...
  make USE_SSL=1 >/dev/null
)

UPSTREAM_LIBHIREDIS_PATH=""
UPSTREAM_LIBHIREDIS_SSL_PATH=""
if [[ -f "$upstream_hiredis_build/libhiredis.dylib" ]]; then
  UPSTREAM_LIBHIREDIS_PATH="$upstream_hiredis_build/libhiredis.dylib"
  [[ -f "$upstream_hiredis_build/libhiredis_ssl.dylib" ]] && UPSTREAM_LIBHIREDIS_SSL_PATH="$upstream_hiredis_build/libhiredis_ssl.dylib"
elif [[ -f "$upstream_hiredis_build/libhiredis.so" ]]; then
  UPSTREAM_LIBHIREDIS_PATH="$upstream_hiredis_build/libhiredis.so"
  [[ -f "$upstream_hiredis_build/libhiredis_ssl.so" ]] && UPSTREAM_LIBHIREDIS_SSL_PATH="$upstream_hiredis_build/libhiredis_ssl.so"
else
  echo "failed to locate upstream libhiredis shared library in $upstream_hiredis_build" >&2
  exit 1
fi

extract_defined_symbols "$UPSTREAM_LIBHIREDIS_PATH" "$UPSTREAM_PROVIDED_SYMS_FILE"
if [[ -n "$UPSTREAM_LIBHIREDIS_SSL_PATH" ]]; then
  extract_defined_symbols "$UPSTREAM_LIBHIREDIS_SSL_PATH" "$UPSTREAM_PROVIDED_SSL_SYMS_FILE"
else
  : > "$UPSTREAM_PROVIDED_SSL_SYMS_FILE"
fi
...
grep -E '^(redis|sds|hi_|hiredis)' "$UPSTREAM_PROVIDED_SSL_SYMS_FILE" > "$UPSTREAM_PROVIDED_SSL_SYMS_FILE.filtered" || true
mv "$UPSTREAM_PROVIDED_SSL_SYMS_FILE.filtered" "$UPSTREAM_PROVIDED_SSL_SYMS_FILE"

comm -23 "$UPSTREAM_PROVIDED_SYMS_FILE" "$PROVIDED_SYMS_FILE" > "$UPSTREAM_MISSING_SYMS_FILE"
comm -23 "$UPSTREAM_PROVIDED_SSL_SYMS_FILE" "$PROVIDED_SSL_SYMS_FILE" > "$UPSTREAM_MISSING_SSL_SYMS_FILE"
...
compat_headers=(
  ...
  "$REPO_ROOT/crates/redis-web-hiredis-compat/include/hiredis/sockcompat.h"
)

if [[ -f "$upstream_hiredis_src/hiredis_ssl.h" && -f "$REPO_ROOT/crates/redis-web-hiredis-compat/include/hiredis/hiredis_ssl.h" ]]; then
  upstream_headers+=("$upstream_hiredis_src/hiredis_ssl.h")
  compat_headers+=("$REPO_ROOT/crates/redis-web-hiredis-compat/include/hiredis/hiredis_ssl.h")
fi
...

{
  ...
  [[ -n "$LIBHIREDIS_SSL_PATH" ]] && echo "compat ssl library: $LIBHIREDIS_SSL_PATH"
  echo "upstream library: $UPSTREAM_LIBHIREDIS_PATH"
  [[ -n "$UPSTREAM_LIBHIREDIS_SSL_PATH" ]] && echo "upstream ssl library: $UPSTREAM_LIBHIREDIS_SSL_PATH"
  echo "extension: $EXT_PATH"
  echo "required symbols file: $REQUIRED_SYMS_FILE"
  echo "provided symbols file: $PROVIDED_SYMS_FILE"
  echo "provided ssl symbols file: $PROVIDED_SSL_SYMS_FILE"
  echo "provided all symbols file: $PROVIDED_ALL_SYMS_FILE"
  ...
  echo "upstream ssl symbols file: $UPSTREAM_PROVIDED_SSL_SYMS_FILE"
  echo "missing upstream ssl symbols file: $UPSTREAM_MISSING_SSL_SYMS_FILE"
...
echo "- missing upstream-exported symbols in compat libhiredis: $(wc -l < "$UPSTREAM_MISSING_SYMS_FILE" | tr -d ' ')"
echo "- missing upstream-exported symbols in compat libhiredis_ssl: $(wc -l < "$UPSTREAM_MISSING_SSL_SYMS_FILE" | tr -d ' ')"
...
if [[ "$STRICT_UPSTREAM_PARITY" == "1" ]]; then
  if [[ -s "$UPSTREAM_MISSING_SYMS_FILE" || -s "$UPSTREAM_MISSING_SSL_SYMS_FILE" || -s "$HEADER_MISSING_API_FILE" ]]; then
    echo
    echo "STRICT_UPSTREAM_PARITY=1 and upstream parity gaps were found." >&2
    if [[ -s "$UPSTREAM_MISSING_SYMS_FILE" ]]; then
      echo "Missing upstream-exported libhiredis symbols:" >&2
      cat "$UPSTREAM_MISSING_SYMS_FILE" >&2
    fi
    if [[ -s "$UPSTREAM_MISSING_SSL_SYMS_FILE" ]]; then
      echo "Missing upstream-exported libhiredis_ssl symbols:" >&2
      cat "$UPSTREAM_MISSING_SSL_SYMS_FILE" >&2
    fi
